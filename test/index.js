import {displayMovieInfo} from './movieInfoDisplay.js';
import {Downloader} from './downloader.js';
import {buildUrlList} from './sample-urls.js';
import {DEVNET} from './ddc-client.js';

/* Setting the level of logs (error, warning, info, debug) */
Log.setLogLevel(console.debug);
...
const bucketId = 324n;
const videoCid = 'bafk2bzacec2pwrqdetdxosv2jdrjkbkpn2fxt3ru5yrkov5z7mckfqn5jrzzo';
const clusterAddress = 1n;
const smartContract = DEVNET;
...
const mainPiece = await downloader.mainPiece;
downloader.setup(mainPiece);
console.log({mainPiece});
let overlayTracks;

let autoplay = false;

let startButton, loadButton, initButton, initAllButton, playButton;
let urlInput, chunkTimeoutInput, chunkSizeInput;
let infoDiv, dlTimeoutDiv;
let chunkTimeoutLabel, chunkSizeLabel, segmentSizeLabel, extractionSizeLabel;
let urlSelector;
let saveChecked;
let progressbar;
let progresslabel;
let html5MediaDiv;
let playbackRateLabel;
let chunkDownloadBitRate;
let cues;
...
function onSourceClose(e) {
    ...
    if (ms.video.error) {
        ...
        console.debug("MSE", "Source closed, no error");
    }
}
...
function addBuffer(video, mp4track) {
    ...
    const codec = mp4track.codec;
    const mime = 'video/mp4; codecs=\"' + codec + '\"';
    const kind = mp4track.kind;
    let trackDefault;
    const trackDefaultSupport = (typeof TrackDefault !== "undefined");
    let html5TrackKind = "";
    if (codec === 'wvtt') {
    ...
    if (MediaSource.isTypeSupported(mime)) {
        try {
            console.debug(`MSE - SourceBuffer #${track_id}`, `Creation with type '${mime}'`);
...
function removeBuffer(video, track_id) {
    ...
    var sb;
    var ms = video.ms;
    console.debug(`MSE - SourceBuffer #${track_id}`, 'Removing buffer');
    var foundSb = false;
    ...
    return ms.sourceBuffers.length === 0;
}
...
function load() {
    ...
    mp4boxfile.onReady = function (info) {
        ...
        if (info.isFragmented) {
            ms.duration = (info.fragment_duration || info.duration) / info.timescale;
        } else {
    ...
    mp4boxfile.onSegment = function (id, user, buffer, sampleNum, is_last) {
        ...
        console.debug("Application", `Received new segment for track ${id} up to sample #${sampleNum}, segments pending append: ${sb.pendingAppends.length}`);
    ...
    mp4boxfile.onSamples = function (id, user, samples) {
        ...
        var cue;
        var texttrack = user;
        console.debug(`TextTrack #${id}`, `Received ${samples.length} new sample(s)`);
        return;
        for (let j = 0; j < samples.length; j++) {
            ...
            if (sample.description.type === "wvtt") {
                ...
                const xmlSubSample = sampleParser.parseSample(sample);
                console.debug(`Parsed XML sample at time ${Log.getDurationString(sample.dts, sample.timescale)} :`, xmlSubSample.document);
    ...
    downloader.setCallback(
        function (response, end, error) {
            if (response) {
                ...
                mp4boxfile.appendBuffer(response, end);
            }
            if (end) {
                progressbar.progressbar({value: 100});
                mp4boxfile.flush();
            } else {
                downloader.setChunkStart(response.byteLength + response.fileStart);
            }
            if (error) {
                reset();
...
function onSeeking(e) {
    ...
    if (video.lastSeekTime !== video.currentTime) {
        ...
        console.debug('Application', `Seeking called to video time ${Log.getDurationString(video.currentTime)}`);
        downloader.stop();
        resetCues();
